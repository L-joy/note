...
在块级作用域内声明函数，应该写成函数表达式，而不是函数声明语句。
...
ES6 一方面规定，为了保持兼容性，`var`命令和`function`命令声明的全局变量，依旧是顶层对象的属性；另一方面规定，`let`命令、`const`命令、`class`命令声明的全局变量，不属于顶层对象的属性。也就是说，从 ES6 开始，全局变量将逐步与顶层对象的属性脱钩。
...
现在有一个提案，在语言标准的层面，引入`globalThis`作为顶层对象。
